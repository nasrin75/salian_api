using salian_api.Config.Permissions;
using salian_api.Dtos.Feature;
using salian_api.Response;
using salian_api.Response.Feature;
using salian_api.Services.Feature;

namespace salian_api.Routes
{
    public static class Feature
    {
        public static void MapFeatureRoutes(this IEndpointRouteBuilder app, string tag)
        {
            var route = app.MapGroup("/api/feature").WithTags(tag);

            route
                .MapGet(
                    "/",
                    async (IFeatureService service) =>
                    {
                        BaseResponse<List<FeatureListResponse>> result = await service.GetAll();
                        return result.ToResult();
                    }
                )
                .RequireAuthorization(new PermissionAuthorizeAttribute(Permissions.Feature.GetAll));

            route
                .MapGet(
                    "/{Id:long}",
                    async (IFeatureService service, long Id) =>
                    {
                        BaseResponse<FeatureResponse?> result = await service.GetByID(Id);
                        return result.ToResult();
                    }
                )
                .RequireAuthorization(
                    new PermissionAuthorizeAttribute(
                        Permissions.Feature.GetAll,
                        Permissions.Feature.Edit
                    )
                );

            route
                .MapPost(
                    "/create",
                    async (IFeatureService service, FeatureCreateDto dto) =>
                    {
                        BaseResponse<FeatureResponse> result = await service.Create(dto);
                        return result.ToResult();
                    }
                )
                .RequireAuthorization(new PermissionAuthorizeAttribute(Permissions.Feature.Create));

            route
                .MapPost(
                    "/search",
                    async (IFeatureService service, SearchFeatureDto dto) =>
                    {
                        BaseResponse<List<FeatureResponse>> result = await service.Search(dto);
                        return result.ToResult();
                    }
                )
                .RequireAuthorization(new PermissionAuthorizeAttribute(Permissions.Feature.GetAll));

            route
                .MapPut(
                    "/edit",
                    async (IFeatureService service, FeatureUpdateDto dto) =>
                    {
                        BaseResponse<FeatureResponse?> result = await service.Update(dto);
                        return result.ToResult();
                    }
                )
                .RequireAuthorization(new PermissionAuthorizeAttribute(Permissions.Feature.Edit));

            route
                .MapDelete(
                    "/delete",
                    async (IFeatureService service, long id) =>
                    {
                        BaseResponse result = await service.Delete(id);
                        return result.ToResult();
                    }
                )
                .RequireAuthorization(new PermissionAuthorizeAttribute(Permissions.Feature.Delete));
        }
    }
}
