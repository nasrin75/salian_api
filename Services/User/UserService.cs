...
namespace salian_api.Services.User
{
    ...
    ) : IUserService
    {
        ...
        {
            var existUsername = _dbContext
                .Users.AsNoTracking()
                .Where(u => u.Username.ToLower().Trim() == dto.Username.ToLower().Trim())
                .Count();
            if (existUsername > 0)
                return new BaseResponse<UserResponse>(null, 400, "USERNAME_IS_EXIST");
            ...
            if (!string.IsNullOrEmpty(dto.IpWhiteLists))
            {
                ...
                // return response
                whiteLists = newUser
                    .IpWhiteLists.Select(x => new IpWhiteListResponse
                    {
                        Id = x.Id,
                        Ip = x.Ip,
                        IpRange = x.IpRange,
        ...
        public async Task<BaseResponse> Delete(long id)
        {
            ...
            if (user == null)
                return new BaseResponse<UserResponse?>(null, 400, "USER_NOT_FOUND");
        ...
        public async Task<BaseResponse<UserResponse?>> GetUserByID(long userID)
        {
            UserEntity? user = await _dbContext
                .Users.Include(x => x.IpWhiteLists)
        ...
        public async Task<BaseResponse<UserResponse?>> Update(UserUpdateDto dto)
        {
            ...
            if (dto.IsCheckIp != null)
                user.IsCheckIp = dto.IsCheckIp.Value;
            if (dto.LoginTypes != null)
                user.LoginTypes = dto
                    .LoginTypes.Select(x => Enum.Parse<LoginTypes>(x)) // true برای case-insensitive
                    .ToList();
            ;
            if (dto.Status != null)
                user.Status = (StatusLists)dto.Status;
            ...
            var existeIps = await _dbContext
                .IpWhiteLists.Where(x => x.UserId == dto.Id)
                .ToListAsync();
            var removeIps = existeIps
                .Where(db => !parsedIps.Any(x => x.Ip == db.Ip && x.IpRange == db.IpRange))
                .ToList();

            var newIps = parsedIps
                .Where(db => !existeIps.Any(x => x.Ip == db.Ip && x.IpRange == db.IpRange))
                .ToList();

            if (removeIps.Any())
                _dbContext.IpWhiteLists.RemoveRange(removeIps);
            if (newIps.Any())
                _dbContext.IpWhiteLists.AddRange(newIps);

            await _dbContext.SaveChangesAsync();

            // response
            List<IpWhiteListResponse> whiteLists = user
                .IpWhiteLists.Select(x => new IpWhiteListResponse
                {
                    Id = x.Id,
                    Ip = x.Ip,
        ...
        public async Task<BaseResponse<List<UserListResponse>>> Search(UserSearchDto param)
        {
            ...
            if (!string.IsNullOrWhiteSpace(param.Username))
                query = query.Where(x => x.Username.Contains(param.Username));
            if (!string.IsNullOrWhiteSpace(param.Email))
                query = query.Where(x => x.Email.Contains(param.Email));
            if (!string.IsNullOrWhiteSpace(param.Mobile))
                query = query.Where(x => x.Mobile.Contains(param.Mobile));
        ...
        private static List<IpWhiteListEntity> ParseIp(string ipWhiteLists, long userID)
        {
            ...
            foreach (var ip in IpLists)
            {
                ...
                {
                    result.Add(new IpWhiteListEntity { Ip = item, UserId = userID });
                }
            }
